...
<script type="text/javascript">
	function startUpdate() {
		...
		frm.method = "get";
		frm.action = "<%=request.getContextPath()%>/myboardUpdate.jsp";
		frm.submit();
	}
	function goDelete() {
		var frm = document.noticeDetail;
		if (confirm("정말 삭제하시겠습니까??") == true){    //확인
			frm.method = "post";
			frm.action = "<%=request.getContextPath()%>/myboardDelete.do";
			frm.submit();
		}else{   //취소
		    return;
		}
			...
			<form name="noticeDetail">
				<div class="CSq1_cont_board">
					...
					<div class="b_button2">
						...
						<button type="button" class="b_button" onclick="location.href='<%=request.getContextPath()%>/myboardList.do'">글목록</button>
					</div>
				</div>
			</form>
		</div>
