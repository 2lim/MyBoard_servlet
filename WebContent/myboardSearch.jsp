	...
	<div id="c_main">
		...
		<div class="page_section">
			<div class="head_aticle">
				<h2 class="tit">
					게시판 <span class="tit_sub"> ' ${mbSearch} ' (으)로 검색한 결과입니다. </span>
				</h2>
			</div>
									...
									<c:if test="${not empty searchList}">
										...
										<c:forEach items="${searchList}" var="v" varStatus="s">
											...
											<tr>
												<td>${v.ncount}</td>
												<td><a
													href="<%=request.getContextPath()%>/myboardDetail.do?nno=${v.nno}&pageNum=${s.count}">${v.ntitle}</a></td>
												<td>${v.ndate}</td>
												<td>${v.nview}</td>
											</tr>
										</c:forEach>
			...
			<div class="pagediv">
				<c:if test="${startPage != 1}">
					<a href="<%=request.getContextPath()%>/myboardSearch.do?pageNum=${prev}"
						class="layout-pagination-button layout-back"><</a>
				</c:if>
				<c:if test="${startPage != endPage}">
					<c:forEach varStatus="s" begin="${startPage}" end="${endPage}"
						step="1">
						<a href="<%=request.getContextPath()%>/myboardSearch.do?pageNum=${s.current}&nSearch=${nSearch}"
							class="layout-pagination-button">${s.current}</a>
						<!--변경 : href 경로 -->
					</c:forEach>
				</c:if>
				<c:if test="${next < pageCount}">
					<a href="<%=request.getContextPath()%>/myboardSearch?pageNum=${next}&msearchbar=${msearchbar}"
		...
		<table class="xans-board-search xans-board-search2">
			<tbody>
				<tr>
					<td>
						<div class="search_bt">
							<form name="noticeSearch" action="<%=request.getContextPath()%>/myboardSearch.do">
